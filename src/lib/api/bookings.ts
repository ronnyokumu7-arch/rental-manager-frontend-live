...
export const bookingsApi = {
  ...
  // ⚡ TRIGGERS: on_booking_created (Generates Quotation, Contract, and Dispatch tasks)
  ...
  // ── Lifecycle Transitions (Event-Driven Triggers) ──────────────────────────
  
  //  TRIGGERS: on_booking_confirmed (Sends Contract task)
  confirm: (id: number) =>
    apiClient.post<Booking>(`/bookings/${id}/confirm`).then((r) => r.data),

  // ⚡ TRIGGERS: on_trip_started (Monitors Return task)
  activate: (id: number) =>
    apiClient.post<Booking>(`/bookings/${id}/activate`).then((r) => r.data),

  // ⚡ TRIGGERS: on_trip_completed (Inspection & Final Invoice tasks)
  complete: (id: number) =>
    apiClient.post<Booking>(`/bookings/${id}/complete`).then((r) => r.data),

  // ⚡ TRIGGERS: on_booking_cancelled (Refund/Release task)
  cancel: (id: number) =>
