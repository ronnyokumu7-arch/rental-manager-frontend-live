...
import { Calendar, ChevronRight, User, Banknote } from "lucide-react";
...
export default function VehicleBookingsTab({ vehicleId }: { vehicleId: number }) {
        ...
        return (
          <div className="min-w-0">
            <p className="text-sm font-medium text-gray-900 truncate">
              {client ? client.full_name : `Client #${row.original.client_id}`}
            </p>
            <p className="text-xs text-gray-500 truncate">{client?.phone || "No phone"}</p>
          </div>
        );
      },
    },
  ...
  return (
    <SectionCard className="!p-0 bg-white border-slate-200 shadow-sm overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Booking History</h3>
      </div>
      {bookings.length === 0 ? (
        <div className="p-8 text-center text-gray-500 text-sm">No bookings found for this vehicle.</div>
      ) : (
        <>
