...
import { Car, Pencil, Save, X, Archive, AlertTriangle, CheckCircle, RotateCcw, Flag } from "lucide-react";
...
interface VehicleHeaderProps {
  ...
  onAction: (action: "activate" | "maintenance" | "reactivate" | "retire" | "archive") => void;
}
    ...
    <div className="bg-[var(--color-surface)] border border-[var(--color-surface-border)] rounded-2xl p-6 shadow-[var(--shadow-card)]">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isEditing ? (
            <>
              <button onClick={() => setIsEditing(false)} className="p-2.5 rounded-xl text-[var(--color-ink-muted)] hover:bg-[var(--color-surface-hover)] transition-all active:scale-95">
                <X size={18} />
              </button>
              <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] transition-all active:scale-95">
