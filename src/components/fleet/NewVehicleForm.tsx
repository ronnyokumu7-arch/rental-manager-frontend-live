...
import { Car, DollarSign, Shield, CheckCircle, FileText, AlertCircle, Loader2, ChevronDown, Calendar, Gauge, Hash, StickyNote } from "lucide-react";
...
const selectClass = "w-full px-3 py-2 rounded-lg border border-[var(--color-surface-border)] bg-[var(--color-surface)] text-[var(--color-ink)] focus:ring-2 focus:ring-[var(--color-primary)]/30 focus:border-[var(--color-primary)] outline-none transition-all text-sm appearance-none cursor-pointer";
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                <select value={formData.make} onChange={(e) => updateField("make", e.target.value)} className={`${selectClass} pr-7`} required>
                  <option value="">Select Make</option>
                  {Object.keys(CAR_DATA).map((make) => <option key={make} value={make}>{make}</option>)}
                </select>
                <ChevronDown size={12} className="absolute right-2 top-2.5 text-[var(--color-ink-subtle)] pointer-events-none" />
              </div>
            </div>
            <div>
              <label className={labelClass}>Model <span className="text-[var(--color-danger)]">*</span></label>
              <div className="relative">
                <select value={formData.model} onChange={(e) => updateField("model", e.target.value)} disabled={!formData.make} className={`${selectClass} pr-7 disabled:opacity-50`} required>
            ...
            <div>
              ...
              <div className="relative">
                <select value={formData.year} onChange={(e) => updateField("year", e.target.value)} className={`${selectClass} pr-7`} required>
            ...
            <div>
              ...
              <div className="relative">
                <Hash size={12} className="absolute left-2.5 top-2.5 text-[var(--color-ink-subtle)]" />
                <input type="text" value={formData.plate_number} onChange={(e) => updateField("plate_number", e.target.value.toUpperCase())} placeholder="KDA 123A" className={`${inputClass} pl-7`} required />
              </div>
            </div>
            <div className="col-span-2">
              <label className={labelClass}>VIN (Optional)</label>
              <input type="text" value={formData.vin} onChange={(e) => updateField("vin", e.target.value.toUpperCase())} placeholder="17-character vehicle identification number" className={inputClass} />
            </div>
          </div>
        </section>
        ...
        <section className={sectionClass}>
          ...
          {hasInsurance ? (
            <div className="mb-3 p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center gap-2">
              ...
              <p className="text-[10px] text-emerald-400 font-medium">Vehicle can be activated immediately after onboarding.</p>
            </div>
          ) : (
            <div className="mb-3 p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-center gap-2">
              <AlertCircle size={12} className="text-amber-500" />
              <p className="text-[10px] text-amber-400 font-medium">Add insurance details to enable immediate activation.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <input type="text" value={formData.insurance_number} onChange={(e) => updateField("insurance_number", e.target.value)} placeholder="POL-12345678" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Insurance Expiry Date</label>
              <div className="relative">
                <Calendar size={12} className="absolute left-2.5 top-2.5 text-[var(--color-ink-subtle)]" />
                <input type="date" value={formData.insurance_expiry} onChange={(e) => updateField("insurance_expiry", e.target.value)} className={`${inputClass} pl-7`} />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <FileUploadSlot label="Insurance" icon={FileText} file={insuranceFile} setFile={setInsuranceFile} />
              <FileUploadSlot label="Registration" icon={FileText} file={registrationFile} setFile={setRegistrationFile} />
              <FileUploadSlot label="Inspection" icon={FileText} file={inspectionFile} setFile={setInspectionFile} />
            </div>
          </div>
        </section>
      ...
      <aside className="lg:sticky lg:top-4 space-y-3">
        ...
        <section className={sectionClass}>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input type="number" step="0.01" min="0" value={formData.daily_rate} onChange={(e) => updateField("daily_rate", e.target.value)} placeholder="0.00" className={inputClass} required />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={labelClass}>Current Mileage</label>
                <div className="relative">
                  <Gauge size={12} className="absolute left-2.5 top-2.5 text-[var(--color-ink-subtle)]" />
                  <input type="number" min="0" value={formData.current_mileage} onChange={(e) => updateField("current_mileage", e.target.value)} placeholder="0" className={`${inputClass} pl-7`} />
                </div>
              </div>
              <div>
                <label className={labelClass}>Next Service</label>
                <input type="number" min="0" value={formData.next_service_km} onChange={(e) => updateField("next_service_km", e.target.value)} placeholder="km" className={inputClass} />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className={sectionClass}>
          ...
          <textarea 
            ...
            rows={3} 
            className="w-full px-3 py-2 rounded-lg border border-[var(--color-surface-border)] bg-[var(--color-surface)] text-[var(--color-ink)] placeholder-[var(--color-ink-subtle)] focus:ring-2 focus:ring-[var(--color-primary)]/30 focus:border-[var(--color-primary)] outline-none transition-all text-sm resize-none" 
