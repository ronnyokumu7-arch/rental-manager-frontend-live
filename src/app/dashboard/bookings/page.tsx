...
import { CalendarDays, ChevronRight, Plus, Car, User, Archive, LayoutList } from "lucide-react";
...
export default function BookingsPage() {
  ...
  // ✅ LOCKED DOWN: Exactly 7 bookings per page to eliminate internal scrolling
  const pageSize = 7; 
  ...
  // ✅ Booking Counters Calculations (Active bookings only, naturally refreshes as data updates)
  const upcomingBookings = bookings.filter(b => !b.is_archived && (b.status === 'pending' || b.status === 'confirmed')).length;
  const activeBookings = bookings.filter(b => !b.is_archived && b.status === 'active').length;
  const completedBookings = bookings.filter(b => !b.is_archived && b.status === 'completed').length;

  return (
    <div className="space-y-6">
      
      {/* Premium Header with Integrated Tab Switcher */}
        ...
        <div className="animate-in fade-in duration-300">
          ...
          <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-surface-border)] shadow-[var(--shadow-card)] overflow-hidden">
            ...
            <div className="p-4 border-b border-[var(--color-surface-border)] bg-[var(--color-surface-hover)]/50 flex flex-col sm:flex-row gap-3 items-center justify-between">
              ...
              {/* ✅ BOOKING COUNTERS: Integrated into toolbar, minimal design */}
              {view === "active" && (
                <div className="flex items-center gap-6 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="text-[var(--color-ink-muted)] font-medium">Pending</span>
                    <span className="text-[var(--color-warning-text)] font-bold tabular-nums">{upcomingBookings}</span>
                  </div>
                  <div className="w-px h-4 bg-[var(--color-surface-border)]" />
                  <div className="flex items-center gap-2">
                    <span className="text-[var(--color-ink-muted)] font-medium">Active</span>
                    <span className="text-[var(--color-success-text)] font-bold tabular-nums">{activeBookings}</span>
                  </div>
                  <div className="w-px h-4 bg-[var(--color-surface-border)]" />
                  <div className="flex items-center gap-2">
                    <span className="text-[var(--color-ink-muted)] font-medium">Completed</span>
                    <span className="text-[var(--color-ink)] font-bold tabular-nums">{completedBookings}</span>
                  </div>
                </div>
              )}
                          ...
                          >
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3 min-w-0">
                                {/* ✅ PREMIUM ICON: No background, blends seamlessly with the brand theme */}
                                <CalendarDays size={18} className="text-[var(--color-primary)] flex-shrink-0 opacity-80" />
